module ApplicationHelper
  ...
  def sidebar_link_to(label, path, icon: nil)
    ...
    css = if active
      "flex items-center gap-3 rounded-lg bg-gray-100 px-3 py-2 text-sm font-medium text-gray-900"
    else
    ...
    link_to path, class: css do
      icon_svg = if icon && SIDEBAR_ICONS[icon]
        raw('<svg class="h-5 w-5 shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">' + SIDEBAR_ICONS[icon] + "</svg>")
      end
      safe_join([ icon_svg, label ].compact)
    end
  end

  def timezone_options
    ActiveSupport::TimeZone.all.map { |tz| [ tz.to_s, tz.tzinfo.canonical_identifier ] }
  end
end
